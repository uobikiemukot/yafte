...
/* misc */
int euc_strlen(const uint8_t c[], int n)
{
	int i, ret = 0;
	uint8_t code;
	...
	for (i = 0; i < n; i++) {
		...
		if (code <= 0x7F) // ASCII
			ret++;
		else if (code == 0x8E) { // JIS X 0201
			ret++;
			i += 1; // skip following 1 byte
		}
		else if (0xA1 <= code && code <= 0xFE) { // JIS X 0208
			ret++;
			i += 1;
		}
		else if (code == 0x8F) { // JIS X 0212
			ret++;
			i += 2; // skip following 2 bytes
		}
	}
...
void euc_delete_char(struct list_t **list)
{
	uint8_t code;
	...
	while (*list != NULL) {
		...
		if (code <= 0x7F || code == 0x8E || code == 0x8F) // ASCII or JIS X 0201 or JIS X 0212
			return;
		else if (0xA1 <= code && code <= 0xFE) { // JIS X 0208
			list_erase_back(list);
			return;
...
int mbyte_strlen(const uint8_t c[], int n)
{
	if (ENCODING == UTF8)
		return utf8_strlen(c, n);
	else if (ENCODING == EUCJP)
		return euc_strlen(c, n);
	else
		fatal("unknown encoding");
}

void mbyte_delete_char(struct list_t **list)
{
	if (ENCODING == UTF8)
		utf8_delete_char(list);
	else if (ENCODING == EUCJP)
		euc_delete_char(list);
	else
		fatal("unknown encoding");
}
...
int write_list(int fd, struct list_t **list, int size)
{
	...
	return mbyte_strlen((uint8_t *) str, size);
}
