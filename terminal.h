...
void addch(struct terminal *term, uint32_t code)
{
	...
	if (width <= 0) /* zero width */
		return;
	else if (code >= MAX_CHARS || fonts[code].width == 0) /* missing glyph */
...
bool push_esc(struct terminal *term, uint8_t ch)
{
	...
	*term->esc.bp++ = ch;
	...
	else if (term->esc.state == STATE_DSCS) {
		if ('0' <= ch && ch <= '~')
			return true;
...
void term_init(struct terminal *term, int width, int height, enum rotate_mode rotate)
{
	int i;
	...
	if (rotate == CLOCKWISE || rotate == COUNTER_CLOCKWISE)
		swap_res(&term->width, &term->height);
