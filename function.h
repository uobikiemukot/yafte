...
void enter_dscs(struct terminal *term, void *arg)
{
	struct parm_t *pt = (struct parm_t *) arg;
	int argc = pt->argc;
...
void enter_sixel(struct terminal *term, void *arg)
{
	/*
		DECDLD header:
		...
		DCS Pfn ; Pcn ; Pe ; Pcmw ; Pss ; Pt ; Pcmh ; Pcss { Dscs
			...
			Pe: Erase control (0, 1, 2)
				0: erase all character in DRCS buffer specified by Dscs
				1: erase only reloaded charater
				2: erase all character
	...
	*/
	struct parm_t *pt = (struct parm_t *) arg;
	char **argv = pt->argv, *cp;

	cp = strstr(term->esc.buf, "{");
	if (cp == NULL) {
		reset_esc(term);
		return;
	}
	...
	term->drcs_index = *cp;
	term->drcs[term->drcs_index].start_char = atoi(argv[1]); // TODO: not implemented
	term->drcs[term->drcs_index].erase_mode = atoi(argv[2]); // TODO: not implemented
	term->drcs[term->drcs_index].charset_num = (atoi(argv[7]) == 0) ? 94: 46; // TODO: not implemented

	if (term->drcs[term->drcs_index].chars == NULL)
		term->drcs[term->drcs_index].chars = (struct static_glyph_t *)
			emalloc(sizeof(struct static_glyph_t) * GLYPH_PER_CHARSET);

	term->esc.state = STATE_SIXEL;
}

void parse_sixel(struct terminal *term, uint8_t ch)
{
}
